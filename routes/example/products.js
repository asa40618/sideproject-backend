...
// 評價資料
router.get('/album/evaluate/:pid', async (req, res) => {
  const sql = `SELECT album_evaluate.id,album_evaluate.album_id,album_evaluate.stars,album_evaluate.comment,album_evaluate.comment_time,member.name FROM album_evaluate INNER JOIN member ON album_evaluate.member_id = member.id where album_id =${req.params.pid}`
  try {
    ...
    res.status(500).json({ error: 'Database query error' }) // 發生錯誤時，返回500狀態碼和錯誤信息
  }
})
